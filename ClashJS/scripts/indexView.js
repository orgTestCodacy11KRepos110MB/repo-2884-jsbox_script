  ...
  $ui.render({
    ...
    views: [{
      ...
      layout: (make, view) => {
        ...
        make.height.equalTo(view.super).offset(-160 - ($device.isIphoneX ? 40 : 0))
      },
      events: {
        didSelect: async (sender, indexPath, data) => {
          ...
          if (data.checkedIcon.icon === null) {
            ...
            let success = await _data.switchProxy(address, sec, data.proxyName.text)
            if (success) {
          ...
          let proxyNowArr = await _data.proxyNow(address)
          $cache.set('proxyNow', proxyNowArr)
        },
...
let loadData = async () => {
  let address = $("urlInputView").text
  // 恢复选中策略
  let proxyNow = $cache.get('proxyNow') || []
  await Promise.all(proxyNow.map(pn => _data.switchProxy(address, pn.name, pn.now)))
