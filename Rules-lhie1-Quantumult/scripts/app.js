...
const settingKeys = ['dnsSettings', 'proxyGroupSettings', 'customSettings', 'hostSettings', 'urlrewriteSettings', 'headerrewriteSettings', 'hostnameSettings']
...
function renderAdvanceUI() {
    ...
    let genControlBnts = function (idx) {
        let titleTexts = ['DNS', '代理分组', '代理规则', '本地DNS映射', 'URL重定向', 'Header修改', '主机名']
...
function makeConf(params) {
    ...
    try {
        ...
        let autoGroup = flatServerData.filter(i => i.proxyName.bgcolor).map(i => i.proxyName.text).join('\n') || 'DIRECT'
        ...
        let promiseArray = [
            getAutoRules(pu.apple, onPgs), // 0
            getAutoRules(pu.reject, onPgs),  // 1
            getAutoRules(pu.proxy, onPgs), // 2
            getAutoRules(pu.direct, onPgs), // 3
            getAutoRules(pu.host, onPgs), // 4
            getAutoRules(pu.urlrewrite, onPgs), // 5
            getAutoRules(pu.urlreject, onPgs), // 6
            getAutoRules(pu.hostname, onPgs) // 7
        ]

        if (!ads) {
            promiseArray[1] = emptyPromise(onPgs)
        ...
        Promise.all(promiseArray).then(v => {
            ...
            rules += `\n${v[0]}\n${v[1]}\n${v[2]}\n${v[3]}\n`
            host = v[4]
            urlRewrite = v[5].split(/[\n\r]/g)
            .map(i => i.trim()).filter(i => {
                let part = i.split(/\s/g)
                return part.length === 3
            }).map(i => {
                let part = i.split(' ')
                return part[2] === '302' ? `${part[0]} url 302 ${part[1]}` : `${part[0]} url modify ${part[1]}`
            }).join('\n')
            urlReject += v[6]
            hostName = v[7].split('\n')
            ...
            let confBlock = function (name, content) {
                return `[${name}]\n${content}\n`
            }

            // 配置代理分组
            if (advanceSettings.proxyGroupSettings) {
                let proxyGroup = advanceSettings.proxyGroupSettings
                proxyGroup = proxyGroup.replace(/Proxy Header/g, proxyHeaders)
                proxyGroup = proxyGroup.replace(/ProxyHeader/g, autoGroup)
                let pp = proxyGroup.replace(/\[POLICY\]\r\n/, '').split('\r\n\r\n')
                console.log(pp.map(i => $text.base64Encode(i)))
                prototype += confBlock('POLICY', pp.map(i => $text.base64Encode(i)).join('\n'))
            }
            // 配置dns设置
            if (advanceSettings.dnsSettings) {
                prototype = prototype + advanceSettings.dnsSettings
            }
            // 配置自定义规则
            ...
            prototype += confBlock('rule', prettyInsert(customRules.add) + rules + '\nGEOIP,CN,🍂 Domestic \nFINAL,☁️ Others\n')
            prototype += confBlock('Proxy', proxies)
            prototype += confBlock('HOST', host + prettyInsert(userHost.add))
            prototype += confBlock('REWRITE', urlRewrite + prettyInsert(userUrl.add))
            prototype += confBlock('URL-REJECTION', urlReject)
            prototype += confBlock('MITM', 'hostname = ' + hostName.concat(userHostname.add.filter(i => i != '')).join(', '))
